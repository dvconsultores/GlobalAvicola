/**
 * P-02 — PROGENITORAS · PRODUCCIÓN DE HUEVO   (`spec.md §4.4`)
 *
...
const P = 'P02'
const TIPO = 'grandparent'

test.describe('P-02 · Progenitoras — producción de huevo', () => {
  test('HAPPY PATH · recolección, clasificación y despacho dentro del saldo', async ({ request }) => {
    const cab = await cabeceraAdmin(request)
    const esc = await crearEscenario(request, cab, P, 'grandparent')
  ...
  test('AISLAMIENTO · el mismo registro vale en la empresa propia y no en la ajena', async ({ request }) => {
    const cab = await cabeceraAdmin(request)
    const mio = await crearEscenario(request, cab, P, 'grandparent')
    await registrar(request, cab, {
    ...
    const suyo = await crearEscenario(request, adminAlla, `${P}B`, 'grandparent')
