...
def test_ac14_sin_migracion_ni_rutas_nuevas():
    ...
    assert ScriptDirectory.from_config(Config(str(raiz / "alembic.ini"))).get_heads() == ["s9t0u1v2w3x4"]
    assert sum(1 for p, _, _ in enumerar_rutas(app) if p.startswith("/api/")) == 208
