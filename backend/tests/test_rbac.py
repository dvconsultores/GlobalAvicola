...
SOLO_SUPER_ADMIN = {
    ...
    ("operations", "delete"),
    # `GA-REM-041-A` · `OD-19` Aclaración A: el propietario asignó `reversals:create/read` al
    # «Supervisor Avícola»; dejan de ser exclusivos de la autoridad global (15 → 13).
    ...
    # Super Administrador. El diagnóstico de este guardián era correcto: faltaba un rol.
}
...
def test_los_permisos_de_super_admin_no_son_una_lista_creciente():
    ...
    **El límite vuelve a 15.** Subió a 17 en la fase 7 para alojar los cuatro permisos de
    `business_units`, que ningún rol concedía, y el guardián tenía razón: faltaba un rol. El
    propietario lo decidió en `OD-15 §6` —«Administrador de Accesos»—, las semillas lo crean,
    y las cuatro entradas salen de la excepción.

    Es la primera vez en este programa que esta lista **baja**. Conviene que quede dicho:
    la salida correcta de un guardián que se queja no es subirle el tope, es contestar lo que
    está señalando.
    """
