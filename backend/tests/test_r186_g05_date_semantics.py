"""R-186 — Semántica temporal del índice de producción G-05.

Contrato canónico (G-05, `app/reports/router.py` + `app/reports/service.py`):

    Broiler production index = (avg_weight_g × viability_pct) / (age_days × FCR × 10)

R-186: `get_kpi_production_index` evaluaba `date.today() - lot.start_date`
mezclando un `datetime.date` con el `datetime` con zona que devuelve
`DateTime(timezone=True)` ⇒ `TypeError` ⇒ **HTTP 500** en cualquier lote con
`start_date` (todo alta fija). La corrección normaliza el día de calendario con
`_dia(...)` (`R-75` / `GA-REM-028`) — el mismo helper canónico que R-184 usa en
`get_kpi_ipe`.

Diferencias con G-06 (IPE) que este contrato PRESERVA (no se reinterpretan):
peso sin filtro de tipo de evento · sin clamp del mismo día (edad 0 ⇒ guarda ⇒
índice 0) · FCR ausente ⇒ `or 1` · fallback de `start_date` nulo = 30.

Nota de ejecución: requiere PostgreSQL (suite canónica). En local sin
credenciales de siembra (`GA_TEST_ADMIN_PASSWORD`, vía `test_credentials`) queda
`skipped` — declarado, no fingido; corre en CI con `backend/scripts/run_tests.sh`.
Evidencia RED/GREEN runtime: `audit/ga-r186/`.
"""
...
# Fixture determinista (independiente de la implementación bajo prueba):
DIAS_EDAD = 19
POBLACION = 1000
MUERTES = 50              # 5 % ⇒ viabilidad 95.0
PESO_MEDIO_G = 2000.0
ALIMENTO_KG = 2500.0      # FCR simplificado = 2500 / 1000 = 2.5

# Esperado a mano desde el contrato documentado:
#   viabilidad = 100 − (50/1000 × 100) = 95.0
#   PI = (2000.0 × 95.0) / (19 × 2.5 × 10) = 190000 / 475 = 400.0
PI_ESPERADO = 400.0
CLAVES_RESPUESTA = {
    "lot_id", "avg_weight_g", "viability_pct", "age_days", "fcr",
    "production_index", "unit",
}
...
async def g05(test_database_url, test_credentials):
    """Fixture autocontenido. `test_credentials` primero: sin credenciales de
    siembra (local) la suite queda `skipped` sin tocar la base."""
    ...
    async with async_sessionmaker(motor, expire_on_commit=False)() as s:
        ...
        if fase is None:
            fase = ProductivePhase(name=f"{PREFIJO}F", code=f"G5{uuid.uuid4().hex[:4]}",
        ...
        def _u(marca: str, role_id: int) -> User:
            return User(first_name=marca, last_name="G05",
...
def _url(lote_id: int) -> str:
    return f"/api/v1/reports/kpis/production-index?lot_id={lote_id}"


# ═══════════════════════════════════════════════════════════════════════════
# Camino feliz determinista — R186-AC06…AC11, AC15
# ═══════════════════════════════════════════════════════════════════════════

async def test_r186_ac06_ac08_determinista(http_client, g05):
    r = await http_client.get(_url(g05["lote_ok"]), headers=_cab(g05["u_ok"]))
    assert r.status_code == 200, f"R-186 sigue devolviendo {r.status_code}: {r.text}"
    d = r.json()
    assert set(d) == CLAVES_RESPUESTA
    assert d["lot_id"] == g05["lote_ok"]
    assert d["viability_pct"] == 95.0
    assert d["avg_weight_g"] == 2000.0
    assert d["age_days"] == DIAS_EDAD
    assert d["fcr"] == 2.5
    assert d["production_index"] == PI_ESPERADO
    assert d["unit"] == "index"


async def test_r186_ac10_estable(http_client, g05):
    r1 = await http_client.get(_url(g05["lote_ok"]), headers=_cab(g05["u_ok"]))
    r2 = await http_client.get(_url(g05["lote_ok"]), headers=_cab(g05["u_ok"]))
    assert r1.status_code == 200 and r2.status_code == 200
    assert r1.json() == r2.json()


# ═══════════════════════════════════════════════════════════════════════════
# Fronteras temporales — R186-AC12…AC16, AC23–AC27
# ═══════════════════════════════════════════════════════════════════════════

async def test_r186_ac14_hoy_sin_clamp(http_client, g05):
    """Mismo día ⇒ edad 0 ⇒ guarda ⇒ índice 0 (contrato vigente; SIN clamp)."""
    r = await http_client.get(_url(g05["lote_hoy"]), headers=_cab(g05["u_ok"]))
    assert r.status_code == 200, r.text
    d = r.json()
    assert d["age_days"] == 0
    assert d["production_index"] == 0
    assert d["fcr"] == 1  # sin alimento ⇒ ratio 0 ⇒ `or 1` (contrato vigente)


async def test_r186_ac16_sin_inicio_fallback_30(http_client, g05):
    r = await http_client.get(_url(g05["lote_nulo"]), headers=_cab(g05["u_ok"]))
    assert r.status_code == 200, r.text
    d = r.json()
...
async def test_r186_ac23_sin_datos_controlado(http_client, g05):
    r = await http_client.get(_url(g05["lote_hoy"]), headers=_cab(g05["u_ok"]))
    assert r.status_code == 200
    d = r.json()
    assert d["production_index"] == 0  # agregados vacíos = 0; sin NaN/∞


# ═══════════════════════════════════════════════════════════════════════════
# Seguridad — R186-AC20/AC28…AC33
# ═══════════════════════════════════════════════════════════════════════════

async def test_r186_ac20_inexistente_404(http_client, g05):
    r = await http_client.get(_url(999_999_999), headers=_cab(g05["u_ok"]))
    assert r.status_code == 404
    assert r.json()["detail"] == "Lote no encontrado"


async def test_r186_ajeno_404(http_client, g05):
    r = await http_client.get(_url(g05["lote_ajeno"]), headers=_cab(g05["u_ok"]))
    assert r.status_code == 404
    assert r.json()["detail"] == "Lote no encontrado"


async def test_r186_sin_concesion_404(http_client, g05):
    r = await http_client.get(_url(g05["lote_ok"]), headers=_cab(g05["u_sin_concesion"]))
    assert r.status_code == 404


async def test_r186_sin_rbac_403(http_client, g05):
    r = await http_client.get(_url(g05["lote_ok"]), headers=_cab(g05["u_sin_permiso"]))
    assert r.status_code == 403
    assert "reports:read" in r.json()["detail"]


async def test_r186_bu_off_404(http_client, g05, test_database_url):
    motor = create_async_engine(test_database_url)
    async with motor.begin() as c:
...
async def test_r186_global_no_bypassa_bu_off(http_client, g05, test_database_url):
    r_on = await http_client.get(_url(g05["lote_ok"]), headers=_cab(g05["u_global"]))
    ...
    async with motor.begin() as c:
        await c.execute(text("UPDATE company_business_units SET is_enabled = false WHERE id = :id"),
                        {"id": g05["hab_a"]})
    try:
        r_off = await http_client.get(_url(g05["lote_ok"]), headers=_cab(g05["u_global"]))
