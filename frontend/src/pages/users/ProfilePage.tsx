...
export default function ProfilePage() {
 ...
 <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:shadow-none p-6 mb-6">
 <h2 className="font-semibold text-slate-700 dark:text-slate-200 mb-4">{t('profile.personalInfo')}</h2>
 <dl className="space-y-3 text-sm">
 <div className="flex justify-between py-2 border-b border-slate-50"><dt className="text-slate-500 dark:text-slate-400">{t('profile.username')}</dt><dd className="font-medium">{user?.username}</dd></div>
 <div className="flex justify-between py-2 border-b border-slate-50"><dt className="text-slate-500 dark:text-slate-400">{t('profile.name')}</dt><dd className="font-medium">{user?.first_name} {user?.last_name}</dd></div>
 <div className="flex justify-between py-2 border-b border-slate-50"><dt className="text-slate-500 dark:text-slate-400">{t('profile.email')}</dt><dd className="font-medium">{user?.email}</dd></div>
 </dl>
 </div>
 ...
 className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm" />
 {message && <p className={`text-sm ${message === t('profile.passwordUpdated') ? 'text-emerald-600' : 'text-red-600 dark:text-red-400'}`}>{message}</p>}
