          ...
          actions={
            conn && (
              <div className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full ${
                conn.connected ? 'bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 dark:text-emerald-400' : 'bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300'
              }`}>
          ...
          <div className="p-5">
            ...
            {jobs.length === 0 && payloads.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('sap.noActivity', 'Sin actividad reciente')}</p>
              </div>
            ) : (
        ...
        {activeTab === 'pending' && (
          <div className="p-5">
            ...
            {payloads.filter(p => p.status === 'pending' || p.status === 'draft').length === 0 ? (
              <div className="text-center py-8">
                <CheckCircle size={32} className="mx-auto text-emerald-300 dark:text-emerald-400 mb-2" />
                <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('sap.noPending', 'Sin documentos pendientes')}</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-100">
                {payloads.filter(p => p.status === 'pending' || p.status === 'draft').map((p: any) => (
                  <div key={p.id} className="py-3 flex items-center justify-between hover:bg-slate-50 dark:bg-slate-800 px-2 rounded-lg transition">
                    <div>
                      <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{p.sap_document_id || `#${p.id}`}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">{p.event_type || t('sap.document', 'Documento')}</p>
                    </div>
          ...
          <div className="p-5">
            ...
            {payloads.filter(p => p.status === 'sent' || p.status === 'confirmed').length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('sap.noSent', 'Sin envíos a SAP')}</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-100">
                {payloads.filter(p => p.status === 'sent' || p.status === 'confirmed').map((p: any) => (
                  <div key={p.id} className="py-3 flex items-center justify-between hover:bg-slate-50 dark:bg-slate-800 px-2 rounded-lg transition">
                    <div>
                      <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{p.sap_document_id || `#${p.id}`}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">{new Date(p.created_at).toLocaleString()}</p>
                    </div>
        ...
        {activeTab === 'errors' && (
          <div className="p-5">
            ...
            {payloads.filter(p => p.status === 'error').length === 0 ? (
              <div className="text-center py-8">
                <CheckCircle size={32} className="mx-auto text-emerald-300 dark:text-emerald-400 mb-2" />
                <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('sap.noErrors', 'Sin errores SAP')}</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-100">
                {payloads.filter(p => p.status === 'error').map((p: any) => (
                  <div key={p.id} className="py-3 hover:bg-red-50 dark:bg-red-950 px-2 rounded-lg transition">
          ...
          <div className="p-5">
            ...
            {jobs.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('sap.noJobs', 'Sin trabajos de sincronización')}</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-100">
                {jobs.map((j: any) => (
                  <div key={j.id} className="py-3 hover:bg-slate-50 dark:bg-slate-800 px-2 rounded-lg transition">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-slate-700 dark:text-slate-200">
