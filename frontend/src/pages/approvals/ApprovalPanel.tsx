    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <SubNavHeader
          ...
          actions={
            <Link to="/review"
              className="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-600 dark:text-slate-300 dark:text-slate-500 hover:text-blue-600 dark:text-blue-400 bg-slate-100 dark:bg-slate-700 hover:bg-blue-50 dark:bg-blue-950 px-4 py-2 rounded-lg transition-colors">
      ...
      <div className="lg:hidden space-y-3">
        ...
        {!loading && events.length === 0 && (
          <div className="text-center py-12">
            <CheckCircle size={40} className="mx-auto mb-2 text-emerald-300 dark:text-emerald-400" />
            <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('review.noPendingApprovals', 'Sin aprobaciones pendientes')}</p>
          </div>
        )}
          ...
          <div key={event.id} className="bg-white dark:bg-slate-800 dark:border-slate-700 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="flex gap-2 mt-3 border-t border-slate-100 pt-3">
              <button onClick={() => openApproveSingle(event.id)}
                className="flex-1 bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 dark:text-emerald-400 px-3 py-2 rounded-lg text-xs font-semibold hover:bg-emerald-100 dark:bg-emerald-950 transition flex items-center justify-center gap-1.5">
                <Check size={14} /> {t('review.approve', 'Aprobar')}
              </button>
              <button onClick={() => openRejectSingle(event.id)}
                className="flex-1 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300 px-3 py-2 rounded-lg text-xs font-semibold hover:bg-red-100 dark:bg-red-950 transition flex items-center justify-center gap-1.5">
                <X size={14} /> {t('review.reject', 'Rechazar')}
              </button>
              <Link to={`/review/${event.id}`}
                className="flex-1 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 dark:text-slate-500 px-3 py-2 rounded-lg text-xs font-semibold hover:bg-slate-100 dark:bg-slate-700 transition text-center flex items-center justify-center gap-1.5">
                <Search size={14} />
              </Link>
            </div>
          </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
            <tr>
              ...
              <th className="px-4 py-3.5 text-left font-semibold text-slate-600 dark:text-slate-300 dark:text-slate-500 text-xs uppercase tracking-wider">ID</th>
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!loading && events.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-12 text-center">
                  <CheckCircle size={28} className="mx-auto mb-2 text-emerald-300 dark:text-emerald-400" />
                  <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('review.noPendingApprovals', 'Sin aprobaciones pendientes')}</p>
                </td>
              </tr>
            )}
            {events.map((event: any) => (
              <tr key={event.id} className="hover:bg-blue-50 dark:bg-blue-950/40 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className="font-mono text-xs font-medium text-slate-600 dark:text-slate-300 dark:text-slate-500 bg-slate-50 dark:bg-slate-800 px-2 py-1 rounded">#{event.lot_id}</span>
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-1.5">
                    <button onClick={() => openApproveSingle(event.id)}
                      className="bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 dark:text-emerald-400 px-2.5 py-1.5 rounded-lg text-xs font-semibold hover:bg-emerald-100 dark:bg-emerald-950 transition flex items-center gap-1">
                      <Check size={12} /> {t('review.approve', 'Aprobar')}
                    </button>
                    <button onClick={() => openRejectSingle(event.id)}
                      className="bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300 px-2.5 py-1.5 rounded-lg text-xs font-semibold hover:bg-red-100 dark:bg-red-950 transition flex items-center gap-1">
                      <X size={12} /> {t('review.reject', 'Rechazar')}
                    </button>
                    <Link to={`/review/${event.id}`}
                      className="bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 dark:text-slate-500 px-2.5 py-1.5 rounded-lg text-xs font-semibold hover:bg-slate-100 dark:bg-slate-700 transition flex items-center">
                      <Search size={12} />
                    </Link>
                  </div>
                </td>
      ...
      {total > limit && (
        <div className="flex items-center justify-center gap-3 mt-5">
          <button
            ...
            className="px-4 py-2 rounded-lg text-sm font-semibold border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 dark:text-slate-500 hover:bg-slate-50 dark:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed transition"
          >
