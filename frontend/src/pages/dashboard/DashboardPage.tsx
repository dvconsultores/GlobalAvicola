...
const ALERT_STYLE: Record<string, { bar: string; bg: string; text: string; badge: string }> = {
 critical: { bar: 'bg-red-500', bg: 'bg-red-50 dark:bg-red-950', text: 'text-red-800 dark:text-red-200', badge: 'bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-300 border-red-200 dark:border-red-800' },
 ...
 info: { bar: 'bg-blue-400', bg: 'bg-blue-50 dark:bg-blue-950', text: 'text-blue-800 dark:text-blue-200', badge: 'bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800' },
}
...
function AlertsWidget({ alerts, onResolve }: {
 ...
 className="shrink-0 text-slate-400 dark:text-slate-500 hover:text-slate-600 p-1 rounded"
...
export default function DashboardPage() {
 ...
 {[1,2,3,4].map(i => (
 <div key={i} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 p-4 h-24 animate-pulse bg-slate-100 dark:bg-slate-700" />
 ))}
 </div>
 </div>
 )
 ...
 <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200/60 p-3">
 <p className="text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wide">{t('dashboard.totalEvents', 'Total eventos')}</p>
 <p className="text-xl font-bold text-slate-800 dark:text-slate-200 mt-1">{data?.total_events ?? 0}</p>
 </div>
 <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200/60 p-3">
 <p className="text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wide">{t('dashboard.last7Days', 'Ultimos 7 dias')}</p>
 <p className="text-xl font-bold text-blue-700 dark:text-blue-300 mt-1">{data?.last_7_days ?? 0}</p>
 </div>
 <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200/60 p-3">
 <p className="text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wide">{t('dashboard.pendingReview', 'Pendientes de revision')}</p>
 <p className="text-xl font-bold text-amber-600 mt-1">{data?.pending_review ?? 0}</p>
 </div>
 <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200/60 p-3">
 <p className="text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wide">{t('dashboard.pendingApproval', 'Pendientes de aprobacion')}</p>
 ...
 <CardBody>
 {totalActiveLots === 0 ? (
 <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-3">{t('lots.noLots', 'Sin lotes activos')}</p>
 ) : (
 ...
 <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200/80 p-3 text-center">
 <div className="text-xl font-bold text-slate-800 dark:text-slate-200">{data?.today_events ?? 0}</div>
 <div className="text-xs text-slate-400 dark:text-slate-500 mt-0.5 font-semibold uppercase tracking-wide">
 ...
 className="w-full bg-white dark:bg-slate-800 rounded-xl border border-slate-200/80 p-3.5 flex items-center gap-3 active:bg-slate-50 transition-colors text-left"
 >
 ...
 {expandedBird === group.id ? <ChevronDown size={16} className="text-slate-400 dark:text-slate-500" /> : <ChevronRight size={16} className="text-slate-400 dark:text-slate-500" />}
 </button>
 ) : (
 <Link
 to={stagePathForKey(group.stages[0].key)}
 className="w-full bg-white dark:bg-slate-800 rounded-xl border border-slate-200/80 p-3.5 flex items-center gap-3 active:bg-slate-50 transition-colors"
 >
 <span className={`w-9 h-9 rounded-lg bg-gradient-to-br ${group.color} flex items-center justify-center shrink-0`}>
 <group.icon size={18} className="text-white" />
 </span>
 ...
 className="flex items-center gap-2.5 p-3 bg-white dark:bg-slate-800 border border-slate-200/80 rounded-lg active:bg-slate-50 transition-colors"
 >
 <span className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 ${isRearing ? 'bg-emerald-50 dark:bg-emerald-950 text-emerald-600' : 'bg-amber-50 dark:bg-amber-950 text-amber-600'}`}>
 ...
 <span className="text-xs text-slate-400 dark:text-slate-500 ml-2">{count} {t('process.hub.steps', 'pasos')}</span>
 </div>
 ...
 { dot: 'bg-blue-500', icon: 'text-blue-600 bg-blue-50 dark:bg-blue-950' },
 { dot: 'bg-amber-500', icon: 'text-amber-600 bg-amber-50 dark:bg-amber-950' },
 { dot: 'bg-teal-500', icon: 'text-teal-600 bg-teal-50' },
 { dot: 'bg-emerald-500', icon: 'text-emerald-600 bg-emerald-50 dark:bg-emerald-950' },
 ]
