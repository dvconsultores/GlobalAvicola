...
export default function CorrectionForm() {
  ...
  if (loading) return <div className="max-w-2xl mx-auto px-4 py-8 text-center text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('common.loading')}</div>
  if (!event) return <div className="max-w-2xl mx-auto px-4 py-8 text-center text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('review.eventNotFound')}</div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      <div className="flex items-center gap-3 mb-6">
        <Link to={`/review/${id}`} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300 dark:text-slate-500">← {t('common.back')}</Link>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 dark:border-slate-700 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 dark:shadow-none p-6 space-y-5">
        {/* Field selector */}
        <div>
          ...
          <select value={fieldName} onChange={e => { setFieldName(e.target.value); setOriginalValue(event[e.target.value] || '') }}
            className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-[#1E3A5F] focus:border-[#1E3A5F]">
            {fields.map(f => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
            <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase mb-1">{t('review.originalValue')}</label>
            <input type="text" value={originalValue} onChange={e => setOriginalValue(e.target.value)}
              className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-800" />
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{t('review.originalByOperator')}</p>
          </div>
          <div className="bg-amber-50 dark:bg-amber-950 rounded-lg p-4 border border-amber-200 dark:border-amber-800">
            <label className="block text-xs font-semibold text-amber-700 dark:text-amber-300 uppercase mb-1">{t('review.correctedValue')}</label>
            <input type="text" value={correctedValue} onChange={e => setCorrectedValue(e.target.value)}
              className="w-full border border-amber-300 dark:border-amber-700 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-800 focus:ring-2 focus:ring-amber-400" />
            <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">{t('review.correctedByYou')}</p>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea value={reason} onChange={e => setReason(e.target.value)}
            ...
            className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-[#1E3A5F]"
            required />
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{t('review.correctionAuditNote')}</p>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Link to={`/review/${id}`}
            className="bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-slate-200 dark:bg-slate-600 transition">
            {t('common.cancel')}
          </Link>
        </div>
      </form>
