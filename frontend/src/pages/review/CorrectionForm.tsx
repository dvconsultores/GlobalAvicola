...
export default function CorrectionForm() {
 ...
 if (loading) return <div className="max-w-2xl mx-auto px-4 py-8 text-center text-slate-500 dark:text-slate-400">{t('common.loading')}</div>
 if (!event) return <div className="max-w-2xl mx-auto px-4 py-8 text-center text-slate-500 dark:text-slate-400">{t('review.eventNotFound')}</div>
 ...
 <Link to={`/review/${id}`} className="text-slate-400 dark:text-slate-500 hover:text-slate-600">← {t('common.back')}</Link>
 ...
 <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:shadow-none p-6 space-y-5">
 {/* Field selector */}
 <div>
 ...
 <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-1">{t('review.originalValue')}</label>
 <input type="text" value={originalValue} onChange={e => setOriginalValue(e.target.value)}
 className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-800" />
 ...
 className="w-full border border-amber-300 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-800 focus:ring-2 focus:ring-amber-400" />
 ...
 <Link to={`/review/${id}`}
 className="bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-slate-200 transition">
 {t('common.cancel')}
 </Link>
 </div>
 </form>
