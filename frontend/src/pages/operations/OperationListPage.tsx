...
export default function OperationListPage() {
 ...
 : events.length === 0 ? <p className="text-slate-400 dark:text-slate-500 text-sm py-8 text-center">{t('common.noResults')}</p>
 : <div className="space-y-3">
 {events.map((ev: any) => (
 <div key={ev.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:shadow-none p-4 flex items-center justify-between">
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2 mb-1">
 <ClipboardList size={20} className="text-slate-400 dark:text-slate-500" aria-hidden="true" />
 <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{getEventLabel(t, ev.event_type)}</span>
 <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[ev.status] || 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300'}`}>{ev.status}</span>
 </div>
