...
export default function LotListPage() {
 ...
 className={`px-3 py-1.5 rounded-lg text-sm font-medium transition ${!birdType ? 'bg-[#1E3A5F] text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200'}`}>
 {t('common.all')}
 </button>
 {BIRD_TYPE_KEYS.map((key) => (
 <button key={key} onClick={() => setBirdType(birdType === key ? '' : key)}
 className={`px-3 py-1.5 rounded-lg text-sm font-medium transition ${birdType === key ? 'bg-[#1E3A5F] text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200'}`}>
 {t(`birdTypes.${key}`)}
 </button>
 ))}
 </div>
 ...
 {loading && <p className="text-slate-500 dark:text-slate-400 text-center py-8">{t('common.loading')}</p>}
 ...
 <p className="text-sm text-slate-600 dark:text-slate-300">{t(`birdTypes.${lot.bird_type}`, String(lot.bird_type || t('lots.noType')))}</p>
 ...
 {/* Desktop Table */}
 <div className="hidden lg:block bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:shadow-none overflow-hidden">
 <table className="w-full text-sm">
 <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200">
 <tr>
 <th className="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">{t('lots.code')}</th>
 <th className="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">{t('lots.type')}</th>
 <th className="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">{t('lots.status')}</th>
 <th className="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">{t('lots.start')}</th>
 <th className="px-4 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">{t('lots.actions')}</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-slate-100">
 {loading && <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-500 dark:text-slate-400">{t('common.loading')}</td></tr>}
 {!loading && lots.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-500 dark:text-slate-400">{t('lots.noLots')}</td></tr>}
 {lots.map((lot: any) => (
 ...
 <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{lot.start_date || '—'}</td>
 <td className="px-4 py-3">
 <Link to={`/lots/${lot.id}`}
