...
export default function LotDetailPage() {
 ...
 <Link to="/lots" className="text-slate-400 dark:text-slate-500 hover:text-slate-600 transition-colors">
 <ArrowLeft size={20} />
 </Link>
 ...
 <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[lot.status] || 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300'}`}>
 ...
 <div><span className="text-slate-500 dark:text-slate-400">{t('lots.totalMortality')}:</span> <strong className="text-red-600 dark:text-red-400">{closeResult.total_mortality}</strong></div>
 ...
 ev.status === 'approved' ? 'bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300' :
 ev.status === 'registered' ? 'bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300' :
 ...
 <p className="text-lg font-bold text-blue-700 dark:text-blue-300">{kpis.egg_production.total_eggs}</p>
 ...
 <p className="text-3xl font-bold text-emerald-700 dark:text-emerald-300">{kpiIpe.ipe}</p>
 ...
 <button onClick={() => handleResolveAlert(a.id)} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 shrink-0 p-0.5" title={t('alerts.resolve', 'Resolver')}>
 <X size={12} />
 </button>
 </div>
 ))}
