...
export default function OperationTile({ step, index, lotId, readOnly }: OperationTileProps) {
  ...
  const inner = (
    <>
      ...
      <span className="absolute top-2 left-2 w-6 h-6 rounded-full bg-white/90 dark:bg-dark-card/90 text-[11px] font-extrabold text-slate-700 dark:text-slate-200 flex items-center justify-center shadow-sm ring-1 ring-black/5">
        {index}
      </span>
      ...
      {/* Label */}
      <span className="mt-3 text-[13px] font-bold leading-tight text-center text-slate-700 dark:text-slate-100 line-clamp-2">
        {label}
      </span>
    </>
  )
