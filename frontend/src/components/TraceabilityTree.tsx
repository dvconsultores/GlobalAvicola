...
export function TraceabilityTree({ lotId, birdType }: Props) {
 ...
 <span className="text-slate-400 dark:text-slate-500">{t('traceability.eggs', 'huevos')}</span>
 {b.quantity_received != null && (
 <span className="text-xs text-slate-400 dark:text-slate-500">({t('traceability.received', 'recibidos')}: {b.quantity_received.toLocaleString()})</span>
 )}
 {b.hatchery_lot && (
 <>
 <ArrowRight size={13} className="text-slate-300 dark:text-slate-500 shrink-0" />
 ...
 <span className="text-slate-500 dark:text-slate-400">{new Date(b.dispatch_date).toLocaleDateString()}</span>
 <span className="font-semibold text-slate-700 dark:text-slate-200">{b.quantity_dispatched.toLocaleString()}</span>
 <span className="text-slate-400 dark:text-slate-500">{t('traceability.chicks', 'pollitos')}</span>
 {b.quantity_received != null && (
 <span className="text-xs text-slate-400 dark:text-slate-500">({t('traceability.received', 'recibidos')}: {b.quantity_received.toLocaleString()})</span>
 )}
 ...
 onClick={load}
 disabled={loading}
 className="text-xs text-slate-400 dark:text-slate-500 hover:text-slate-600 flex items-center gap-1 transition-colors"
 >
