...
export function TraceabilityTree({ lotId, birdType }: Props) {
  ...
  if (error) {
    return (
      <p className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 flex items-center gap-1">
        {t('traceability.loadError', 'Error al cargar trazabilidad')}
        <button onClick={load} className="text-blue-500 dark:text-blue-400 hover:underline ml-1">{t('common.retry', 'Reintentar')}</button>
      </p>
    )
  }
  ...
  if (!hasData) {
    return (
      <p className="text-sm text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 italic">
  ...
  return (
    <div className="space-y-5">
      ...
      {data!.egg_batches_sent.length > 0 && (
        <div>
          <h4 className="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <Egg size={13} />
          ...
          <div className="space-y-2">
            {data!.egg_batches_sent.map(b => (
              <div key={b.id} className="flex items-center gap-2 text-sm">
                <span className="text-slate-500 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">{new Date(b.dispatch_date).toLocaleDateString()}</span>
                <span className="font-semibold text-slate-700 dark:text-slate-200">{b.quantity_dispatched.toLocaleString()}</span>
                <span className="text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('traceability.eggs', 'huevos')}</span>
                {b.quantity_received != null && (
                  <span className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">({t('traceability.received', 'recibidos')}: {b.quantity_received.toLocaleString()})</span>
                )}
                {b.hatchery_lot && (
                  <>
                    <ArrowRight size={13} className="text-slate-300 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 shrink-0" />
          ...
          <div className="space-y-2">
            {data!.chick_batches_sent.map(b => (
              <div key={b.id} className="flex items-center gap-2 text-sm">
                <span className="text-slate-500 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">{new Date(b.dispatch_date).toLocaleDateString()}</span>
                <span className="font-semibold text-slate-700 dark:text-slate-200">{b.quantity_dispatched.toLocaleString()}</span>
                <span className="text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('traceability.chicks', 'pollitos')}</span>
                {b.quantity_received != null && (
                  <span className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">({t('traceability.received', 'recibidos')}: {b.quantity_received.toLocaleString()})</span>
                )}
      ...
      >
        <div className="space-y-4">
          ...
          />
          {linkError && <p className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950 rounded-lg px-3 py-2">{linkError}</p>}
        </div>
      </Modal>
