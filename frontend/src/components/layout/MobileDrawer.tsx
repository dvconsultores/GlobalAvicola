...
export default function MobileDrawer({ open, onClose }: MobileDrawerProps) {
 ...
 {/* Panel */}
 <nav className={`lg:hidden fixed top-0 left-0 h-full w-[88vw] max-w-[340px] z-50 flex flex-col bg-white dark:bg-slate-800 transition-transform duration-200 ease-out ${open ? 'translate-x-0' : '-translate-x-full'}`}>
 {/* Header */}
 ...
 className="col-span-2 flex items-center gap-3 p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 active:bg-slate-100 transition-colors">
 ...
 className="flex flex-col items-center gap-2 p-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 active:bg-slate-100 dark:active:bg-slate-700 transition-colors text-center">
 ...
 <span className="text-[10px] text-slate-900 dark:text-slate-100">{items.length} {t('common.options', 'opciones')}</span>
 </div>
 </button>
 )
 })}
 ...
 className="w-full flex items-center gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-100 active:bg-slate-100 dark:active:bg-slate-700 transition-colors"
 >
 {Icon && <Icon size={18} className="text-blue-600 shrink-0" />}
 <div className="flex-1 min-w-0 text-left">
 <span className="text-sm font-semibold text-slate-900 dark:text-slate-100 block truncate">{t(item.labelKey, item.fallback)}</span>
 <span className="text-[10px] text-slate-900 dark:text-slate-100">{item.children.length} {t('common.options', 'opciones')}</span>
 </div>
 <ArrowLeft size={14} className="text-slate-900 dark:text-slate-100 rotate-180" />
 </button>
 </div>
 )
 }
 ...
 className="flex items-center gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-100 active:bg-slate-100 dark:active:bg-slate-700 transition-colors">
 {Icon && <Icon size={18} className="text-blue-600 shrink-0" />}
 ...
 {user && (
 <p className="text-[10px] text-slate-900 dark:text-slate-100 px-3 text-center">
