...
- **Actualización (2026-09-13 15:25 +0200) — AC-06 observado**: con la sesión autorizada del propietario se leyeron directamente los runs ⇒ **están ROJOS** (incluido `66be1c1`) ⇒ **AC-06 = FAIL observado** (la declaración previa `success` queda contradicha). Causa backend: `pip install -e` (flat-layout) — **corrección C3** validada y enviada en este commit (solo `.github/workflows/quality-suite.yml`). Frontend: pendiente de su log. Con runs VERDES ⇒ AC-06 = PASS ⇒ T1 cierra. Producto/tests/migraciones sin cambio.
