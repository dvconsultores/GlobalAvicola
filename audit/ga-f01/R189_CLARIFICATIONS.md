...
| C29 | Recepción contra lote autocreado (sin galpón): ¿de dónde sale el `house_id` del evento? | Del **primer galpón declarado** en las filas de «Distribución por galpón» (`bird_movements[i].target_house_id`) **cuando el lote no declara uno**; si el lote declara galpón, manda el del lote. Solo `bird_reception` (frontera explícita; distribución/traslado no se reabren). Sin galpón en ninguno ⇒ ausencia (no se inventa) | retry F-03 (`evidence/runtime-c2d/`) + sonda BR-08 + control lote-con-galpón |

Sin clarificaciones críticas abiertas ⇒ implementación habilitada (C1); ampliación F-01d habilitada tras C2 (anexo §4); ampliación F-01e habilitada tras el retry (anexo F-01e §3).
