...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **F1** | `GET /business-units` → `[]`; 4 códigos → 404 en candidatos. La administración de unidades y concesiones es inerte en ENV-01 | `GA-REM-040 §2` · `OD-16.a` (4 unidades = catálogo de plataforma) · `baseline_seeds.UNIDADES_DE_NEGOCIO` | **A** (sembrar el catálogo) — `GA-FE-02-B §1` · **remediación autónoma autorizada** — `GA-FE-02-C §2` | ENV-01 fue sembrado antes de que existiera el seed del catálogo; `sembrar_unidades_de_negocio` nunca corrió; **no existe API de catálogo** (server-side only) ⇒ la siembra manual/opcional no la garantiza el ciclo de despliegue | **ENV** (data de plataforma) → remediada **en código** (dato determinista) | Bajo (inercia, no exposición) | Bloquea E2E-02…10 + consolas de acceso | **SÍ** (migración de datos `y5z6a7b8c9d0`; seed importa la fuente única) | **NO** (la migración corre en el arranque normal — `GA-REM-024`; sin docker exec/SSH/SQL manual) | NO (inicialización de modelo gobernado) | — (no; registrado como bloqueo operativo de ENV-01) | AC-F1 (spec §4 + §4.2, AC01–17) | **`F1 = FAIL` (3.ª verificación) → `REMEDIACIÓN AUTÓNOMA AUTORIZADA (GA-FE-02-C §2)`** — `GET /business-units` c1/c3 = `[]` (`count=0`), 4×404 candidatos; `is_active default=True` descarta «inactivas»; dos reportes del propietario no observables; `BLOCKED_SERVER_ACCESS_F1` agotado. Remediación: migración de datos `y5z6a7b8c9d0` (spec §4.2) por pipeline normal; postcondición runtime `count=4` pendiente de deploy (AC-F1-13). |
...
**Resolución estructural (`GA-FE-02-C`)**: la causa —una siembra que el ciclo de despliegue no
garantiza— se elimina por diseño (migración de datos `y5z6a7b8c9d0` en el arranque normal,
`GA-REM-024`). El bloqueo de acceso deja de ser ruta crítica: ya no se requiere ejecución
manual en el servidor para F1.
