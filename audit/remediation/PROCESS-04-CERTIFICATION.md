# CERTIFICACIÓN — P-04 · Reproductoras — producción de huevo fértil
...
`spec.md §4.6` · cadena documentada en `audit/06_PROCESS_COVERAGE.md` · regla de negocio
**`BR-02`**.
...
por eso se ejecuta su cadena completa de **10 pasos** y no solo su tramo distintivo.

## 2. Casos ejecutados

`9 / 9` en verde, sobre lote de tipo `breeder`.

| Tipo de caso | Resultado |
|---|---|
| `CADENA COMPLETA` — los 10 pasos documentados | **PASS** |
| `HAPPY PATH` | **PASS** |
| `NEGATIVE PATH` — `BR-02` | **PASS** |
| `VALIDATION` | **PASS** |
...
| `FK OWNERSHIP` | **PASS** |
| `AUDIT` | **PASS** |
| `PERSISTENCE` / `DERIVED STATE` | **PASS** — saldo derivado 340 |
...
anular `BR-02` y las guardas de pertenencia, los casos correspondientes **fallan**. Un
